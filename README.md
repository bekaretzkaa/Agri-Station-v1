...
| Architecture | MVVM, Repository Pattern |
| Local Database | Room / SQLite, TypeConverters |
| Networking | Retrofit 2, OkHttp, Ktor, Kotlinx Serialization |
| Maps | Google Maps SDK for Android |
| AI | Google Gemini API |
| Async | Kotlin Coroutines, Flow, StateFlow |
| Dependency Injection | Manual DI through `AppContainer` |
...
1. Clone the repository:
   ...
   ```bash
   git clone https://github.com/YOUR_USERNAME/agristation.git
   cd agristation
   ```

2. Add API keys and backend configuration to `local.properties`:
...
3. If you are using the mock server, start it:
